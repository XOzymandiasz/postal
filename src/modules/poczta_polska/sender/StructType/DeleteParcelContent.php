...
class DeleteParcelContent extends AbstractStructBase
{
    ...
    public static function validateParcelContentForArrayConstraintFromSetParcelContent(?array $values = []): string
    {
        ...
        $message = '';
        $invalidValues = [];
        foreach ($values as $deleteParcelContentParcelContentItem) {
            // validation for constraint: itemType
            if (!$deleteParcelContentParcelContentItem instanceof \XOzymandias\Yii2Postal\modules\poczta_polska\sender\StructType\ParcelContentType) {
                $invalidValues[] = is_object($deleteParcelContentParcelContentItem) ? get_class($deleteParcelContentParcelContentItem) : sprintf('%s(%s)', gettype($deleteParcelContentParcelContentItem), var_export($deleteParcelContentParcelContentItem, true));
            }
        }
     ...
     * @param \XOzymandias\Yii2Postal\modules\poczta_polska\sender\StructType\ParcelContentType[] $parcelContent
     * @return \XOzymandias\Yii2Postal\sender\StructType\DeleteParcelContent
