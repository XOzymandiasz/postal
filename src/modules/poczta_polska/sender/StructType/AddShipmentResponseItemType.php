...
class AddShipmentResponseItemType extends AbstractStructBase
{
     ...
     * @var string|null
     */
    protected ?string $numerListuPrzewozowego = null;
    /**
    ...
    {
        // validation for constraint: string
        if (!is_null($numerListuPrzewozowego) && !is_string($numerListuPrzewozowego)) {
            throw new InvalidArgumentException(sprintf('Invalid value %s, please provide a string, %s given', var_export($numerListuPrzewozowego, true), gettype($numerListuPrzewozowego)), __LINE__);
        }
