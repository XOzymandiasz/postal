...
class DeleteParcelContentResponse extends AbstractStructBase
{
    ...
    public static function validateErrorForArrayConstraintFromSetError(?array $values = []): string
    {
        ...
        $message = '';
        $invalidValues = [];
        foreach ($values as $deleteParcelContentResponseErrorItem) {
            // validation for constraint: itemType
            if (!$deleteParcelContentResponseErrorItem instanceof \XOzymandias\Yii2Postal\modules\poczta_polska\sender\StructType\ErrorType) {
                $invalidValues[] = is_object($deleteParcelContentResponseErrorItem) ? get_class($deleteParcelContentResponseErrorItem) : sprintf('%s(%s)', gettype($deleteParcelContentResponseErrorItem), var_export($deleteParcelContentResponseErrorItem, true));
            }
        }
     ...
     * Set error value
     * @param \XOzymandias\Yii2Postal\modules\poczta_polska\sender\StructType\ErrorType[] $error
     * @return \XOzymandias\Yii2Postal\sender\StructType\DeleteParcelContentResponse
