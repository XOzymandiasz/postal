...
class UpdateChecklistTemplateResponse extends AbstractStructBase
{
    ...
    public static function validateErrorForArrayConstraintFromSetError(?array $values = []): string
    {
        ...
        $message = '';
        $invalidValues = [];
        foreach ($values as $updateChecklistTemplateResponseErrorItem) {
            // validation for constraint: itemType
            if (!$updateChecklistTemplateResponseErrorItem instanceof \XOzymandias\Yii2Postal\modules\poczta_polska\sender\StructType\ErrorType) {
                $invalidValues[] = is_object($updateChecklistTemplateResponseErrorItem) ? get_class($updateChecklistTemplateResponseErrorItem) : sprintf('%s(%s)', gettype($updateChecklistTemplateResponseErrorItem), var_export($updateChecklistTemplateResponseErrorItem, true));
            }
        }
     ...
     * Set error value
     * @param \XOzymandias\Yii2Postal\modules\poczta_polska\sender\StructType\ErrorType[] $error
     * @return \XOzymandias\Yii2Postal\sender\StructType\UpdateChecklistTemplateResponse
