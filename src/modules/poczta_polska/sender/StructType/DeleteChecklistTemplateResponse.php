...
class DeleteChecklistTemplateResponse extends AbstractStructBase
{
    ...
    public static function validateErrorForArrayConstraintFromSetError(?array $values = []): string
    {
        ...
        $message = '';
        $invalidValues = [];
        foreach ($values as $deleteChecklistTemplateResponseErrorItem) {
            // validation for constraint: itemType
            if (!$deleteChecklistTemplateResponseErrorItem instanceof \XOzymandias\Yii2Postal\modules\poczta_polska\sender\StructType\ErrorType) {
                $invalidValues[] = is_object($deleteChecklistTemplateResponseErrorItem) ? get_class($deleteChecklistTemplateResponseErrorItem) : sprintf('%s(%s)', gettype($deleteChecklistTemplateResponseErrorItem), var_export($deleteChecklistTemplateResponseErrorItem, true));
            }
        }
     ...
     * Set error value
     * @param \XOzymandias\Yii2Postal\modules\poczta_polska\sender\StructType\ErrorType[] $error
     * @return \XOzymandias\Yii2Postal\sender\StructType\DeleteChecklistTemplateResponse
